    ...
    for (const cursoCrudo of cursos) {
      ...
      const result = await pool.query(
        `INSERT INTO publicaciones
           (hash_origen, titulo, descripcion, tipo, fuente, estado,
      ...
      if (result.rowCount > 0) {
        ...
        // Disparar push (no bloqueante, si falla no detiene el worker)
        ...
        }).catch(err => console.error('  ⚠️  Push error (no crítico):', err.message));
